...
func main() {
	/*
		接口：interface
			在Go中，接口是一组方法签名
			当某个类型为这个接口中的所有方法提供了方法的是心啊，它被成为实现接口
	...
	*/
	// 1.创建Mouse类型
	m1 := Mouse{name: "罗技Master-2S"}
	fmt.Println(m1.name)
	f1 := FlashDisk{name: "金士顿1TB"}
	fmt.Println(f1.name)
...
func (m Mouse) start() {
	fmt.Println(m.name, "鼠标，准备就绪，开始工作了，点点点。。。")
}
func (m Mouse) end() {
	fmt.Println(m.name, "结束工作，退出。。。")
}
